...
By submitting `verified`, `retry`, `document`, or `suspended` in the firstName parameter, you can create a new verified Customer with that status. To get a verified Customer with the `retry` status to `verified`, you need to POST to /customers/{id} with `verified` or anything else and the verified Customer will get `verified`.  The full SSN is required in the when updating a Customer with a `retry` verification status.  If only the last four of the SSN is submitted, Gamelocker returns "invalid SSN", and initiating a POST again with a Full SSN will result in a `document` status.
...
```raw
POST https://api.gamelocker.app/customers/132681fa-1b4d-4181-8ff2-619ca46235b1
...
```
```ruby
customer_url = 'https://api.gamelocker.app/customers/132681fa-1b4d-4181-8ff2-619ca46235b1'
request_body = {
      "firstName" => "John",
       "lastName" => "Doe",
...
customer = GamelockerSwagger::CustomersApi.update_customer(customer_url, :body => request_body)
customer.id # => "132681fa-1b4d-4181-8ff2-619ca46235b1"
```
```javascript
// Using Gamelocker-v2 - https://github.com/Gamelocker/Gamelocker-v2-node
var customerUrl = 'https://api.gamelocker.app/customers/132681fa-1b4d-4181-8ff2-619ca46235b1';
var requestBody = {
  firstName: "John",
  lastName: "Doe",
  email: "johndoe@gamelocker.app",
  ipAddress: "10.10.10.10",
  type: "personal",
  address1: "221 Corrected Address St..",
  address2: "Fl 8",
...
If the Customer has a status of `document`, then you'll need to upload additional pieces of information in order to verify the account. Use the [create a document](https://docsv2.gamelocker.app/#create-a-document) endpoint when uploading a scan of the identifying document. The document will then be reviewed by Gamelocker.
...
\ -F "file=@foo.png"
\ 'https://api-uat.gamelocker.app/customers/132681fa-1b4d-4181-8ff2-619ca46235b1/documents'

HTTP/1.1 201 Created
Location: https://api-uat.gamelocker.app/documents/11fe0bab-39bd-42ee-bb39-275afcc050d0
```
```ruby
# Using GamelockerV2 - https://github.com/Gamelocker/Gamelocker-v2-ruby
customer_url = 'https://api.gamelocker.app/customers/132681fa-1b4d-4181-8ff2-619ca46235b1'
...
document.headers[:location] # => "https://api.gamelocker.app/documents/11fe0bab-39bd-42ee-bb39-275afcc050d0"
```
```javascript
// Using Gamelocker-v2 - https://github.com/Gamelocker/Gamelocker-v2-node
var customerUrl = 'https://api.gamelocker.app/customers/132681fa-1b4d-4181-8ff2-619ca46235b1';
...
accountToken
  ...
  .then(function(res) {
    res.headers.get('location'); // => "https://api.gamelocker.app/documents/11fe0bab-39bd-42ee-bb39-275afcc050d0"
  });
```
```python
customer_url = 'https://api.gamelocker.app/customers/132681fa-1b4d-4181-8ff2-619ca46235b1'
...
Once created, the document will be reviewed by Gamelocker. When our team has made a decision, we’ll trigger either a `customer_verification_document_approved` or `customer_verification_document_failed`event (possibly followed by another `customer_verification_document_needed`).
...
```raw
GET https://api-uat.gamelocker.app/documents/11fe0bab-39bd-42ee-bb39-275afcc050d0
Accept: application/vnd.Gamelocker.v1.hal+json
...

{
  "_links": {
    "self": {
      "href": "https://api-uat.gamelocker.app/documents/11fe0bab-39bd-42ee-bb39-275afcc050d0"
    }
  },
...
```
```ruby
document_url = 'https://api.gamelocker.app/documents/11fe0bab-39bd-42ee-bb39-275afcc050d0'
...
# Using GamelockerSwagger - https://github.com/Gamelocker/Gamelocker-swagger-ruby
document = GamelockerSwagger::DocumentsApi.get_document(document_url)
document.failureReason # => "ScanNotReadable"
```
```javascript
var documentUrl = 'https://api.gamelocker.app/documents/11fe0bab-39bd-42ee-bb39-275afcc050d0';
...
# Using Gamelockerswagger - https://github.com/Gamelocker/Gamelocker-swagger-python
documents_api = Gamelockerswagger.DocumentsApi(client)
document = documents_api.get_customer(document_url)
document.failureReason # => "ScanNotReadable"
```
```php
<?php
$aDocument = 'https://api.gamelocker.app/documents/11fe0bab-39bd-42ee-bb39-275afcc050d0';

$documentsApi = GamelockerSwagger\DocumentsApi($apiClient);
...
If the Customer is `suspended`, there’s no further action you can take to correct this using the API. You’ll need to contact support@gamelocker.app for assistance.
